...
def step_wise_decay(epoch, lr, init_lr=0.1):
    # TODO: uncomment next lines, fill the ... spots, and remove `raise NotImplementedError()`
    """
    if epoch < 80:
        return init_lr
    elif epoch < 120:
...
def step_wise_decay_with_warmup(epoch, lr, init_lr=0.1, warmup_epochs=5):
    # TODO: uncomment the next lines, fill the ... spots, and remove `raise NotImplementedError()`
    """
    ...
    if epoch < 5:
        ...
        # first five epochs.
        # As we use the linear scaling rule (see below in the code after `if args.enable_lr_rescaling`), we will multiply further the learning rate
        # by hvd.size(). Thus, coupled with linear scaling, this will result in starting from `init_lr` and linearly
...
def main(args):
    ...
    if args.enable_lr_rescaling:
        # TODO: uncomment next line, fill the ... spot, and remove `raise NotImplementedError()`
        """
    ...
    if args.lr_scheduler == "step_wise_decay":
        ...
        # TODO: uncomment the next line, fill the ... spot, and remove `raise NotImplementedError()`
        """
        callbacks.append(OneCycleLR(50000, batch_size * hvd.size(), max_lr=...))
        """
        raise NotImplementedError()
