"""
Create and retrieve TFRecords of the Tiny-ImageNet-200 dataset.
"""
...
def _parse_args():
    ...
    parser.add_argument(
        'dataset_path',
        nargs='?',
        default='/p/project/training2306/datasets/tiny-imagenet-200.zip',
        help='Dataset file to parse (a ZIP file).',
    )
    parser.add_argument(
        'output_dir',
        nargs='?',
        default='.',
        help='Where to generate the TFRecord files.',
    )
    return parser.parse_args()


def generate_tfrecords(dataset_path, output_dir):
    """Generate TFRecord files at `output_dir` from the Tiny-ImageNet-200
    ZIP file at `dataset_path`.
    """
    dataset_path = Path(dataset_path).expanduser()
    output_dir = Path(output_dir).expanduser()

    train_path = output_dir / 'tiny-imagenet-200-train.tfrecords'
    val_path = output_dir / 'tiny-imagenet-200-val.tfrecords'
    test_path = output_dir / 'tiny-imagenet-200-test.tfrecords'

    loader = TinyImageNetLoader(dataset_path)
...
if __name__ == '__main__':
    args = _parse_args()
    generate_tfrecords(args.dataset_path, args.output_dir)
