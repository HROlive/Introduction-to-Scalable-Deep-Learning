...
- [Zoom](https://go.fzj.de/scalable-dl-may-2023-zoom)
- [Slack](https://go.fzj.de/scalable-dl-may-2023-slack)
- [Compute project](https://go.fzj.de/scalable-dl-may-2023-project)
- [Course page](https://go.fzj.de/scalable-dl-may-2023-course)
- [Latest version of this document](https://go.fzj.de/scalable-dl-may-2023) (Please open it and follow along, we'll need it)
- [Source code for this course](https://gitlab.jsc.fz-juelich.de/MLDL_FZJ/juhaicu/jsc_public/sharedspace/teaching/intro_scalable_deep_learning/course-material-may-2023/-/tree/main/)
...
- Understand the base concepts for the next days
...
::::
:::: {.col}
![Jan Ebert](pics/jan.jpg)
::::
:::: {.col}
![Stefan Kesselheim](pics/stefan.jpg)
::::
:::: {.col}
![Mehdi Cherti](pics/mehdi.jpg)
::::
:::: {.col}
![Jenia Jitsev](pics/jenia.jpg)
::::
...
| 12:15 - 12:30 | Sync (everyone should be at the same point) |
| 12:30 - 13:00 | MPI example |
...
- Supercomputer: A damn lot of computers bound together locally
  - with a very very fancy network
    - which is so fancy, we call it "interconnect"
...
- 780 Nodes, each of them having:
- AMD EPYC Rome 7742 CPU 2.25 GHz (2 × 64 cores / node)
- 512 GiB memory
- 192 nodes with 4x NVIDIA A100 with 40gb.

TL;DR: 768 A100 gpus!!! 😅

Way deeper technical info at [JURECA DC Documentation](https://apps.fz-juelich.de/jsc/hps/jureca/index.html)
...
### Connecting to JUWELS BOOSTER and JURECA DC

#### Getting compute time
- Go to [https://go.fzj.de/scalable-dl-may-2023-project](https://go.fzj.de/scalable-dl-may-2023-project)
- Join the course project `training2306`
- Sign the Usage Agreements ([Video](https://drive.google.com/file/d/1mEN1GmWyGFp75uMIi4d6Tpek2NC_X8eY/view))
- Compute time allocation is based on compute projects. For every compute job, a compute project pays.
- Time is measured in core-hours. One hour of Juwels BOOSTER is 48 core-hours. One hour of Jureca is 128 core-h.
- Example: Job runs for 8 hours on 64 nodes of Juwels BOOSTER: 8 * 64 * 48 = 24576 core-h!

---

### Connecting to JUWELS BOOSTER and JURECA
...
- Go to VSCode and make it simpler, replace the 2nd half with `0.0/16` becomes `93.199.0.0/16`
...
- For FZJ people, add a second magic number: `,134.94.0.0/16` (the internal network)
...
- ```bash
from="93.199.0.0/16,10.0.0.0/8,134.94.0.0/16" ssh-ed25519 AAAAC3NzaC1lZDE1NTA4AAAAIHaoOJF3gqXd7CV6wncoob0DL2OJNfvjgnHLKEniHV6F strube@demonstration.fz-juelich.de
```
...
![](images/manage-ssh-keys-jureca.png)

- Let's add it on [Judoor](https://judoor.fz-juelich.de)
- Do it for JUWELS, JURECA and JUDAC with the same key
...
```bash
$ ssh jureca
The authenticity of host 'jureca.fz-juelich.de (134.94.0.185)' cannot be established.
...
ln -s $PROJECT_training2306 ~/course
...
(make sure you are still connected to JURECA)
...
You can:
- send and receive files: Just drag to/from the file list on the left!
...
- From the terminal view, navigate to your "course" folder and to the name you created earlier.
...
### Demo code
#### Create a new file "`matrix.py`" on VSCode on Jureca
...
``` {.bash .number-lines}
#!/bin/bash -x
#SBATCH --account=training2306           # Who pays?
...
- access the supercomputer both via terminal VSCode 🖥️
- navigate to your project directory 📁
- submit a job 🚀
- wait ⏳
- cancel the job if it was messed up ❌
- check the results 📊
- download them via VSCode 🔽
...
- Use the supercomputers iteractively (I'll be really quick here)
- Install our own software
- Access the compute nodes directly
- Do some parallel programming
- As famously said by \[1\], let's do it in small pieces
  - \[1\]: Lecter, Hannibal. _The Silence of the Lambs_
...
- Edit the file requirements.txt: `code requirements.txt`
...
- Opens a connection on port 3000... *OF THE SUPERCOMPUTER'S LOGIN NODE*.
...
`ssh -L :1234:localhost:3000 jureca`

---

## Port forwarding demo:

- On local computer: `ssh -L :1234:localhost:3000 jureca`
- On jureca:
- ```bash
cd $HOME/course/$USER
source sc_venv_template/activate.sh
...
- A port is exclusive per user. If you get error, log out of the supercomputer and try 3001, 3002 etc
...
- Submit: `sbatch fastai-demo.sbatch`
- Wait: `squeue --me`
- Check logs
...
- ```python
  File "/p/software/jureca/stages/2023/software/Python/3.10.4-GCCcore-11.3.0/lib/python3.10/urllib/request.py", line 1391, in https_open
    return self.do_open(http.client.HTTPSConnection, req,
  File "/p/software/jureca/stages/2023/software/Python/3.10.4-GCCcore-11.3.0/lib/python3.10/urllib/request.py", line 1351, in do_open
    raise URLError(err)
...
- `ssh -L :3334:localhost:5000 jsfc013i.jureca`
